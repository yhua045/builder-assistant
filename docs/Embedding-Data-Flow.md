...
sequenceDiagram
    ...
    UI->>P: Parse source file
    P->>X: Persist extracted text and page/section metadata
    C->>W: Validate state and mark chunking progress
...
The queue/orchestrator is responsible for creating, restoring, and publishing workflow work. The current `RagPipelineOrchestrator` does not directly call parsing, chunking, and embedding use cases; those stages are separate application boundaries coordinated by the surrounding workflow/consumer integration.
...
### 6. `project_facts` (adjacent RAG data)
...
`ReceiveDocumentUseCase` creates a document ID and version workflow record with `workflow_state = document_received` and `status = received`. The document service path additionally persists the `documents` row, storage path, and checksum before publishing work.

### 2. Validate

Validation determines whether the document can be analyzed. A failed validation updates the workflow record with `status = failed`, `workflow_state = failed`, `supported_for_analysis = false`, and a validation reason. No chunks should be generated from a failed document.
...
`ChunkDocumentUseCase` requires a passed validation state and non-empty extracted text. It processes pages, records page-level progress, uses the structured chunking strategy with deterministic fallback, persists `knowledge_chunks`, and updates the workflow to `chunking_complete` or `failed`.
...
Workflow rows carry status, stage, retry, and checkpoint fields. `RagPipelineOrchestrator.restoreQueue()` rehydrates `pending`, `partial`, and `running` runs. Resume/retry must operate against the existing document/version run and must not create duplicate chunks or embeddings for completed work.
