...
- If applicable, map out explicit workflow states and valid state transitions (use a **Mermaid stateDiagram-v2** diagram).

- List side effects, triggers, and guard conditions for each state change.
- Not all features/changes required a workflow mapping. Only the states that are truly relevant to the domain should be mapped out. Otherwise, ignore it
...
- Clarify the responsibilities of each abstraction.
