...
import type { KnowledgeEmbeddingDocumentService } from '../../document-intake/application/services/KnowledgeEmbeddingDocumentService';
import { KnowledgeEmbeddingStep } from '../../workflow/domain/value-objects/KnowledgeEmbeddingStep';
import type { KnowledgeEmbeddingRunView } from '../../document-intake/application/contracts/DocumentIntakeContracts';
...
export function useKnowledgeEmbeddingFlow(): KnowledgeEmbeddingFlowViewModel {
  ...
  const [queueConsumer] = useState(() => container.resolve<KnowledgeEmbeddingQueueConsumer>('KnowledgeEmbeddingQueueConsumer'));
  ...
  useEffect(() => {
    ...
    return queueConsumer.addListener((item) => {
      if (committedDocumentIds.has(item.documentId)) {
        void refreshCommittedRuns([...committedDocumentIds]);
      }
    });
  }, [committedRuns, currentStep, refreshCommittedRuns, queueConsumer]);
