...
export class DrizzleDocumentChunkingWorkflowRepository implements DocumentChunkingWorkflowRepository {
  ...
  async findByDocumentVersion(documentId: string, version: number): Promise<DocumentChunkingWorkflowRecord | null> {
    ...
    return result.rows.length === 0 ? null : this.mapRowToRecord(result.rows.item(0));
  }
  ...
  async findByStatus(status: string): Promise<DocumentChunkingWorkflowRecord[]> {
    ...
    for (let index = 0; index < result.rows.length; index += 1) records.push(this.mapRowToRecord(result.rows.item(index)));
    return records;
  }
  ...
  async deleteByDocumentVersion(documentId: string, version: number): Promise<void> {
    await this.ensureInitialized();
    const { db } = getDatabase();
    await db.executeSql('DELETE FROM knowledge_embedding_runs WHERE document_id = ? AND document_version = ?', [documentId, version]);
  }
}