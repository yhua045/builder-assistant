...
export class PdfTextParser implements DocumentParser {
  ...
  async parse(input: DocumentParseInput): Promise<ParsedDocumentText> {
    ...
    console.info('[knowledge-embedding] PDF parser input', {
      documentId: input.documentId,
      documentVersion: input.documentVersion,
      contentType: input.contentType,
      filePath: input.filePath,
      hasRawText: typeof input.rawText === 'string',
      rawTextLength: input.rawText?.length ?? 0,
      hasBinary: Boolean(input.binary),
      binaryLength: input.binary?.byteLength ?? 0,
    });
